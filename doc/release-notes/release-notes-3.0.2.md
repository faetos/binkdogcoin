...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the BinkDogCoin address as it's first parameter and the passphrase as it's second parameter.
