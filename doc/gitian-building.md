...
*Setup instructions for a gitian build of BinkDogCoin using a Debian VM or physical system.*
...
- [Building BinkDogCoin](#building-binkdogcoin)
...
Clone the git repositories for binkdogcoin and gitian and then checkout the binkdogcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/binkdogcoin-crypto/binkdogcoin.git
cd binkdogcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the binkdogcoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building BinkDogCoin
----------------

To build BinkDogCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the binkdogcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/binkdogcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/binkdogcoin-crypto/binkdogcoin
...
For example:
```bash
URL=https://github.com/crowning-/binkdogcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/binkdogcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/binkdogcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/binkdogcoin-build.assert
```
...
[binkdogcoin/gitian.sigs](https://github.com/binkdogcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
