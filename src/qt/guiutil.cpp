...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BinkDogCoin address (e.g. %1)").arg("S7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no BinkDogCoin: URI
...
{
    // Convert binkdogcoin:// to binkdogcoin:
    //
    //    Cannot handle this later, because binkdogcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open binkdogcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BinkDogCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BinkDogCoin.lnk
...
{
    return GetAutostartDir() / "binkdogcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a binkdogcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BinkDogCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the binkdogcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add binkdogcoin app to startup item list
