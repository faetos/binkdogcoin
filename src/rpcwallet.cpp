...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "2. \"keysobject\"   (string, required) A json array of binkdogcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) binkdogcoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of BinkDogCoin to convert to zHLM\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "\nSpend zHLM to a BinkDogCoin address.\n" +
            HelpRequiringPassphrase() + "\n"
