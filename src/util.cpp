...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "binkdogcoin" is a composite category enabling all BinkDogCoin-related debug output
            if (ptrCategory->count(string("binkdogcoin"))) {
...
#else
    const char* pszModule = "binkdogcoin";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BinkDogCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BinkDogCoin
// Mac: ~/Library/Application Support/BinkDogCoin
// Unix: ~/.binkdogcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BinkDogCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "binkdogcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "binkdogcoind.pid"));
